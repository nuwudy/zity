...
<body class="bg-gray-50 min-h-screen text-gray-900">
    ...
    <header class="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
        <div class="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
            ...
            <div class="flex-1 relative" id="search-container">
                <form action="{{ route('search') }}" method="GET" class="search-bar flex items-center bg-gray-50 border-2 border-gray-200 focus-within:border-indigo-500 rounded-2xl transition-all overflow-hidden">
                ...
                <div class="flex items-start gap-4 p-5">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm text-gray-600 line-clamp-2 leading-relaxed">
                            {{ Str::limit($business->description, 160) }}
                        </p>
                        @endif
                        ...
                        <div class="flex flex-wrap items-center gap-3 mt-3 text-xs text-gray-500">
                            ...
                            @endif
                            @if(!empty($business->branches) && count($business->branches) > 0)
                            <span class="flex items-center gap-1 text-[10px] text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                                {{ count($business->branches) + 1 }} Locations
                            </span>
                            @endif
    ...
    <script>
        ...
        searchInput.addEventListener('keydown', e => {
            ...
            if (e.key === 'ArrowDown' && suggestionsBox.classList.contains('active')) {
                e.preventDefault();
                selectedIndex = Math.min(selectedIndex + 1, items.length - 1);
                updateSelection(items);
            } else if (e.key === 'ArrowUp' && suggestionsBox.classList.contains('active')) {
                e.preventDefault();
                ...
                // If no selection, default form submit happens
